...
function RouteEffects(){const {pathname}=useLocation();const first=useRef(true);useEffect(()=>{document.title=pathname==='/'?titles['/']:`${titles[pathname]||'Page Not Found'} | Hayat Health Care Clinic Pupri`;document.querySelector('meta[property="og:title"]')?.setAttribute('content',document.title);document.querySelector('meta[name="twitter:title"]')?.setAttribute('content',document.title);const origin=import.meta.env.VITE_SITE_URL;if(origin){const base=new URL(import.meta.env.BASE_URL,origin);let canonical=document.querySelector('link[rel="canonical"]');if(!canonical){canonical=document.createElement('link');canonical.rel='canonical';document.head.append(canonical);}canonical.href=new URL(pathname.slice(1),base).href;}if(!first.current){window.scrollTo({top:0,behavior:'instant'});document.getElementById('main')?.focus({preventScroll:true});}first.current=false;},[pathname]);return null;}
