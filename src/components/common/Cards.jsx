...
export function DoctorCard({ doctor:d }) {
  ...
  return <article className="doctor-card">{d.photo?<DoctorPhoto className="card-doctor-photo" {...(typeof d.photo === 'object' ? d.photo : {})}/>:<div className="doctor-avatar" aria-label="Doctor profile avatar"><Icon name="UserRound" size={65}/><span className="avatar-plus">+</span></div>}<div className="doctor-card-body"><span className="eyebrow">YOUR CARE TEAM</span><h3>{d.name}</h3><p className="doctor-specialty">{d.specialty}</p><dl className="doctor-profile-details">{details.map(([label,items])=><div key={label}><dt><Icon name="BadgeCheck" size={14}/>{label}</dt><dd>{items.join(' · ')}</dd></div>)}</dl><Link className="text-link" to={`/appointment?doctor=${encodeURIComponent(d.name)}`}>Consult Doctor<Icon name="ArrowUpRight" size={16}/></Link></div></article>;
}
